...
- `MessageRepositoryIntegrationTests.swift` (4 tests): seeding when empty, skip when non-empty, modifiedAt preservation, fallback behavior
- `FileStorageManagerTests.swift` (7 tests): storage limit enforcement, atomic replace, idempotent delete, temp file cleanup, storage calculation
...
- [x] Integration tests exist for MessageRepository seeding + update semantics.
- [x] Integration tests exist for FileStorageManager overwrite + storage limit behavior.
- [x] Tests are deterministic and clean up any on-disk artifacts they create.
